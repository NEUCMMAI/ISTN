...
from models.difsr.modules import DIFTransformerEncoder
...
class DIFSR(SequentialRecommender):
    ...
    def __init__(self, config, dataset, vaild_data = None, raw_dataset = None, regression=True, only_eval_target=False):
        super(DIFSR, self).__init__(config, raw_dataset)
        ...
        self.feature_embed_layer_list = nn.ModuleList(
            [copy.deepcopy(FeatureSeqEmbLayer(raw_dataset,self.attribute_hidden_size[_],[self.selected_features[_]],self.pooling_mode,self.device)) for _
        ...
        for attribute in self.selected_features:
            self.n_attributes[attribute] = len(raw_dataset.field2token_id[attribute])
