...
class IGCMC(GeneralRecommender):
    
    input_type = InputType.PAIRWISE
    
    def __init__(self, config, dataset, vaild_data = None, raw_dataset = None, regression=True, only_eval_target=False):
        super(IGCMC, self).__init__(config, raw_dataset)

        # load dataset info
        self.num_all = self.n_users + self.n_items
        self.interaction_matrix = dataset.dataset.inter_matrix(form='coo').astype(np.float32)  # csr
        ...
        graph_users = self.Graph._indices()[0,:]
        node_id, counts= torch.unique(graph_users, return_counts = True)
        node_degree = torch.zeros(self.num_all, dtype = torch.long).to(self.device)
        
        # generate node feature
        if self.sparse_feature:
            node_degree[node_id - 1] = counts
            node_idx = torch.tensor([i for i in range(self.num_all)]).to(self.device)
            feature_indices = torch.stack([node_idx, node_degree])
            values = torch.ones_like(node_idx).float()
            self.user_features = torch.sparse_coo_tensor(feature_indices[:, :self.n_users], 
                                                         values[:self.n_users], [self.n_users, self.num_all]).to(self.device)
            item_i = feature_indices[:, self.n_users:]
            item_i[0, :] = item_i[0, :] - self.n_users
            self.item_features = torch.sparse_coo_tensor(item_i, values[self.n_users:], [self.n_items, self.num_all]).to(self.device)
            
        else:
            self.user_features = torch.nn.functional.one_hot(node_degree[:self.n_users]).to(self.device)
            self.item_features = torch.nn.functional.one_hot(node_degree[self.n_users:]).to(self.device)
